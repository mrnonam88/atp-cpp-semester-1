...
//        std::cout << "now count\n";
        if (bust[index] == two_dim_arr[index][0] + 1){
            if (index != 0){
                ...
                bust[index - 1]++;
                index--;
//                print_permutation(bust, index, result_sum);
                permutate(bust, result_sum, index);
            }
        }else{
//            std::cout << index << " " << two_dim_arr_size << std::endl;
...
//    std::cout << "great, now I created arrays, so please fill 'em\n";
    int input_num = 0;
