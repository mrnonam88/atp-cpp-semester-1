...
#include <string>
#include <algorithm>
//Если ты это поймешь, то я тебе очень завидую
class BigInteger{
private:
    ...
    std::vector<long long> unsigned_division(const BigInteger& num) {
        ...
        long long bottom_bound;
        long long current_bound;
        for (long long i = 0; i < size; ++i) {
...
//======================================================================================================================
    ...
    std::string asDecimal(size_t precision = 0) const{
        ...
        if (numerator < 0 && copy/denominator == 0) precised += '-';
    ...
    [[nodiscard]] long long length(int i) const{
        if (i == 1)return numerator.length();
        return denominator.length();
    }
    friend std::ostream& operator<<(std::ostream& output, const Rational& jennifer);
...
std::ostream& operator<<(std::ostream& output, const Rational& jennifer) {
    output.tie(nullptr);
    output << jennifer.toString();
    return output;
}
//######################################################################################################################
//######################################################################################################################
...
struct is_Prime {
    static const bool is_prime = (N == D) || ((N % D != 0) && is_Prime<N, D + 1, D*D >= N>::is_prime);
};
...
template <size_t N>
Residue<N> operator +(const Residue<N>& res_1, const Residue<N>& res_2) {
...
template<size_t N>
std::ostream& operator<<(std::ostream& output, const Residue<N>& jennifer) {
    output.tie(nullptr);
    output << int(jennifer);
    return output;
}
...
private:
    ...
    [[nodiscard]] size_t get_leader_col(size_t i, size_t start) const{
        for (size_t j = start; j < M; ++j) {
            if (matrix[j][i] != Field(0)) return j;
        }
        return M + 1;
    }   //gets first element in column not zero
...
public:
    ...
    [[nodiscard]] Matrix<N, M, Field> transposed() const{
        ...
        for (size_t i = 0; i < N; ++i)  for (size_t j = 0; j < M; ++j) transposed[i][j] = matrix[j][i];
        return transposed;
    }
    template<typename = typename std::enable_if<M == N>>
     Matrix<N, M, Field>& invert(){
        ...
        for (size_t row = 0; row < M; ++row) {
            size_t leader_row = row;
            while ((leader_row < M) && (matrix[leader_row][row] == Field(0))) ++leader_row;     //finding index of column leader
    ...
    std::vector<Field> getRow(size_t i) {return matrix[i];}
    ...
    Matrix<M, N, Field>& operator += (const Matrix<M, N, Field>& matrix1) {
        for (size_t i = 0; i < M; ++i)  for (size_t j = 0; j < N; ++j)  matrix[i][j] += matrix1.matrix[i][j];
        return *this;
    }
    Matrix<M, N, Field>& operator -= (const Matrix<M, N, Field>& matrix1) {
        for (size_t i = 0; i < M; ++i)  for (size_t j = 0; j < N; ++j)  matrix[i][j] -= matrix1.matrix[i][j];
    ...
    Matrix<M, N, Field> triangulated(){return triangulate().first;}
};
//continue
template <size_t M, size_t N, typename Field>   //sum
...
template <size_t M, size_t N, typename Field>   //diff
Matrix<M, N, Field> operator -(const Matrix<M, N, Field>& matrix_1, const Matrix<M, N, Field>& matrix_2) {
    Matrix<M, N, Field> copy(matrix_1);
    return copy -= matrix_2;
}
template <size_t M, size_t N, typename Field, size_t K>     //matrix * matrix
...
template <size_t M, size_t N, typename Field>       //scalar * matrix
